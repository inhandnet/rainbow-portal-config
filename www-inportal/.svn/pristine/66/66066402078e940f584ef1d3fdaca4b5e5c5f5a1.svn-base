...
function initNewTreeHtml(containerID,xmlData)//树形结构创建方法
{
	...
	for(var i=0;i<father.childNodes.length;i++)  //一级(父)DIV 循环
	{
		...
		var dirString=p_label;
		 ...
		 treeUL+="<a value='"+p_value+"' label='"+p_label+"' str='"+dirString+"' id='ulLista_"+p_id+"' class='MenuBarItemSubmenu'><span>"+p_label+"</span>";
...
function createFatherDivs(currentfather){
	...
	for(var i=0;i<currentfather.childNodes.length;i++)
	{
		...
		divTable+="<tr>";
		divTable+="<td width='5px' style=' background-image: url(../images/lableBG05.png)' ></td>";		
		divTable+="<td  style='padding-left:2px;padding-right:4px;background-image:url(../images/lableBG04.png);cursor:pointer' id='td"+c_id+"' label="+c_label+" value="+c_value+" f_id="+f_id+" onclick='tdClick(this)' ><span id='Span_"+f_id+"_td"+c_id+"' name='labelTdName'  style='font-size:12px; font-weight:bold;color:"+initTabColor+"'>"+c_label+"</span></td>";
	    divTable+="<td width='1px' style=' background-color:#333333' ></td>";
	    divTable+="</tr>";
	    divTable+="</table>";
	    divTable+="</td>";    
...
function tdClick(obj){
	...
	document.getElementById('Span_'+f_id+'_'+id).style.color="black";
