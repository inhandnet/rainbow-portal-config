...
//tacacs_server = [['2.2.2.2','1812','1234'],['1.1.1.1','1812','8765'],['','0',''],['','0',''],['','0',''],['','0',''],['','0',''],['','0',''],['','0',''],['','0','']];
...
aaaentrys.setup = function() {
	...
	for (var i = 0; i < tacacs_server.length; i++) {
		this.insertData(-1, [tacacs_server[i][0],tacacs_server[i][1],
					tacacs_server[i][2]
					]);
	}
...
function verifyFields(focused, quiet)
{
	...
	for(var i = 0; i < tacacs_server.length; i++) {
		...
		for(var j = 0; j < data.length; j++) {
			if(data[j][0]==tacacs_server[i][0]) {	//server
				found = 1;
				break;
			}
		}
		if(!found) {
			cmd += "no tacacs-server host " + tacacs_server[i][0] + " port " + tacacs_server[i][1];
			if(tacacs_server[i][2] == '') cmd += "\n";
			else cmd += " key " + tacacs_server[i][2] + "\n";
		}
	}
	...
	for(var i = 0; i < data.length; i++) {
		var found = 0;
		var changed = 0;
		for(var j = 0; j < tacacs_server.length; j++) {
			if(data[i][0]==tacacs_server[j][0]) {	//index
				found = 1;
				if(data[i][1] != tacacs_server[j][1]
					|| data[i][2] != tacacs_server[j][2]) {
					changed = 1;
				}
				break;
			}
		...
		if(!found || changed) {
			cmd += "tacacs-server host " + data[i][0] + " port " + data[i][1];
			if(data[i][2] == '') cmd += "\n";
