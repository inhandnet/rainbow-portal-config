...
var vid_cookie = cookie.get('igmp-modify-vid');
var mac_cookie = cookie.get('igmp-modify-mac');
if (vid_cookie == null){
	vid_cookie = 0;
}
if (mac_cookie == null)
...
var dest_group_config = 	["", "0100.5e00.0001", []];


if (mac_cookie != ""){
	for (var i = 0; i < IGMP_snooping_config.static_member.length; i++){
		if (parseInt(IGMP_snooping_config.static_member[i][0], 10) == vid_cookie
			&& IGMP_snooping_config.static_member[i][1] == mac_cookie){
			dest_group_config = IGMP_snooping_config.static_member[i];
		}
	}
}
...
function groupInVlan(group_config)
{
	for (var i = 0; i < vlan_options.length; i++){
		if (group_config[0] == vlan_options[i][1]){
			return i; 
		}
	}
...
{
	for (var i = 0; i < IGMP_snooping_config.static_member.length; i++){
		if (IGMP_snooping_config.static_member[i][0] == vid
			&& IGMP_snooping_config.static_member[i][1] == mac){
			return 1;
		}
	}
...
function back()
{
	document.location = 'switch-igmp-snooping.jsp';	
}

function port_mem_in_group(port_index, group_config)
{
	for (var i = 0; i < group_config[2].length; i++){
		if(port_id[port_index][0] == group_config[2][i][0]
			&& port_id[port_index][1] == group_config[2][i][1]
			&& port_id[port_index][2] == group_config[2][i][2]){
			return true;
		}
	}	
...
function verifyFields(focused, quiet)
{
	...
	if (mac_cookie == ""){
		//mac
		if (E('_f_mac').value == ''
			|| !v_igmp_mac(E('_f_mac').value)){
			ferror.set('_f_mac', errmsg.mac_igmp, quiet);
			return 0;
		}else{
			ferror.clear('_f_mac');
		}
	...
	for (var i = 0; i < port_title_list.length; i++){
		...
		if (E('_f_port_'+i).checked != port_mem_in_group(i, dest_group_config)){
			cmd += (E('_f_port_'+i).checked == false?"no ":"")+"ip igmp snooping vlan " + vlan_options[E('_f_vid').value][1]+" static "+ E('_f_mac').value + " interface "+port_cmd_list[i]+"\n";
		}
	}
...
var group_tb = [
					{ title: ui.mac_address, name: 'f_mac', type: 'text', maxlen: 14, size: 16, value: dest_group_config[1]},
